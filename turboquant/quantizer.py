...
class TurboQuantMSE(torch.nn.Module):
    ...
    def quantize(self, x: torch.Tensor) -> MSEQuantized:
        ...
        indices = torch.searchsorted(self.decision_boundaries, y.to(self.decision_boundaries.dtype).contiguous())
...
class TurboQuantProd(torch.nn.Module):
    ...
    def quantize(self, x: torch.Tensor) -> ProdQuantized:
        """Quantize vectors x of shape (..., d) to ProdQuantized format.

        [Optim 4] Prefill quantize fusion
          Upstream's path (see 0xSero/turboquant) calls quantize() then
          dequantize() — the latter unpacks the indices we just packed and
          rotates back. We reuse the raw `idx` from searchsorted to gather
          centroids directly, saving one unpack + one gather kernel launch
          per call. Output is bit-exact with upstream (verified by
          test_prefill_batch_quantize_bit_exact).
        """
        # ── Stage 1: MSE quantize fused with residual computation ──
        norms = x.norm(dim=-1, keepdim=False)                                     # (...,)
        x_unit = x / (norms.unsqueeze(-1) + 1e-10)
        y = rotate_forward(x_unit.float(), self.mse_quantizer.Pi)                  # (..., d)

        idx = torch.searchsorted(
            self.mse_quantizer.decision_boundaries,
            y.to(self.mse_quantizer.decision_boundaries.dtype).contiguous(),
        )                                                                            # (..., d)

        # Pack indices ONCE for storage
        packed_idx = _pack_indices(idx, self.mse_quantizer.bits)

        # Reconstruct y_hat directly from the un-packed idx (no unpack needed)
        y_hat = self.mse_quantizer.centroids[idx]                                  # (..., d)

        # x_hat in original space (rotation back + rescale by norms)
        x_unit_hat = rotate_backward(y_hat, self.mse_quantizer.Pi)
        ...
        # ── Stage 2: QJL projection + sign-pack ──
        projected = torch.matmul(residual.float(), self.S.float().T)
        packed_signs = self._pack_qjl_signs(projected)

        return ProdQuantized(
            mse_indices=packed_idx,
    ...
    def dequantize(self, q: ProdQuantized) -> torch.Tensor:
        ...
        signs = self._unpack_qjl_signs(q.qjl_signs).to(self.S.dtype)
    ...
    def attention_score(
        ...
        q_sketched = torch.matmul(query.float(), self.S.float().T)
